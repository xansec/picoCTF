...
try:
    ...
    from ruamel.yaml.comments import (
        CommentedSeq,
    )  # sometimes we get this instead of normal lists
except ImportError:
    print(
...
assert (
    sha512_crypt("CHANGE_AND_REKEY", salt="vGd1X7SV")
    == "$6$vGd1X7SV$yeT9KeCiRudYnbyUfdxgUqDcFUS1XoFZaNoBAypXyYbfSiHnWpg6SQUWWiGK2ux8BFO70Uk2uycUuzX/H7ExA1"
), "mkpasswd implementation failed, try on Linux instead"
...
def edit_tmpvault(filename):
    ...
    master_pass = getpass.getpass("Enter master key to generate values: ").encode(
        "utf-8"
    )
        ...
        if v == old_key or k.endswith("_crypt"):
            ...
            if k.endswith("_crypt"):
                ...
                extra_keys[k + "_clear"] = key[
                    8:
                ]  # add the key in the clear for easy lookup by an admin with the vault key
            else:
                vault_dict[k] = key
